...
class AccessService {
    ...
    static async handleRefreshToken(refreshToken: string) {
        ...
        const holderToken = await KeyTokenService.findTokenByRefreshToken(refreshToken)
        if (!holderToken) {
    ...
    static async handleRefreshTokenV2(keyStore: IKeyTokenModel, user: IUser, refreshToken: string) {
        const { email } = user;
    ...
    static async changePassword(email: string, currentPass: string,
