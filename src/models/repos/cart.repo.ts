...
const addProductIntoCartFunc = async (cartDetail: ICartDetail, idCart: string): Promise<any> => {
    ...
    if (checkProductExists) {
        ...
        checkProductExists.priceCartDetail = checkProductExists.quantity * cartDetail.priceCartDetail
    }
    else {
        ...
        const checkColor = productFound.product_colors.map((e) => e.color_code).includes(color)
        if (!checkColor) {
            throw new BadRequestError("The color does not exist in this product!")
        }
        const checkSize = productFound.product_sizes.map((e) => e.size_name).includes(size)
        if (!checkSize) {
    ...
    foundCart.cart_count = foundCart.cart_products.map((e) => e.quantity).reduce((prev, acc) => acc + prev)
    foundCart.cart_total_price = foundCart.cart_products.map((e) => e.priceCartDetail).reduce((prev, acc) => acc + prev)
...
const updateCart = (cart: ICart): void => {
    cart.cart_count = cart.cart_products.map((e) => e.quantity).reduce((prev, acc) => acc + prev)
    cart.cart_total_price = cart.cart_products.map((e) => e.priceCartDetail).reduce((prev, acc) => acc + prev)
}
...
const decreaseQuantityProductInCartFunc = async (idCartDetail: string, idCart: string): Promise<any> => {
    const foundCart = await findCartById(idCart)
    console.log("idCartDetail: ", idCartDetail)
    if (!foundCart) {
        throw new BadRequestError("Can not find cart to remove product!")
    ...
    if (cartItem.quantity > 1) {
        cartItem.quantity -= 1
        const productCartUpdate: IProduct = await findProductById(cartItem.product.toString())
        const priceProduct = (productCartUpdate.product_price + productCartUpdate.product_sizes.find((s) => s.size_name = cartItem.size)?.size_price! + productCartUpdate.product_colors.find((c) => c.color_code === cartItem.color)?.color_price!)
