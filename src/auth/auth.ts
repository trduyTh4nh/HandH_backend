...
const authentication = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const userId = req.headers[HEADER.CLIENT_ID.toLowerCase()] as string | undefined;

    if (!userId) {
        throw new AuthFailureError('Invalid request!')
    }
    console.log("DEBUG USER ID", userId)
    console.log("Header check", req.headers)
    ...
    console.log("ACCESS TOKEN DEBUG: ", accessToken)
...
const checkRoleAd = asyncHandler(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
